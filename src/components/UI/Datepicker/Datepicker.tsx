...
export class DatePicker extends Component<Props, any> {
	...
	public render() {
		const { minDate, maxDate, cancelBtnText, confirmBtnText } = this.props;
		...
		return (
			<React.Fragment>
				{Platform.OS === 'ios' && (
					...
					>
						<View style={{ flex: 1 }}>
							<Touchable style={styles.datePickerMask}>
								<Touchable style={{ flex: 1 }}>
									...
									>
										...
										>
											<Label style={[ styles.btnTextText ]}>{confirmBtnText}</Label>
										</Touchable>
									</Animated.View>
								</Touchable>
							</Touchable>
				...
				<Touchable onPress={this.dateClickHandler} style={[ styles.inputContainer, this.props.style ]}>
					<View style={styles.label}>
						{this.props.placeholder && (
							...
							>
								{this.props.placeholder}{' '}
								{this.props.required && (
									<Text style={{ color: !this.state.dateString ? colors.red : colors.primary }}>
										*
									</Text>
								)}
							</Label>
					...
					{this.props.uri && <Image source={{ uri: this.props.uri }} style={this.props.iconStyle} />}
				</Touchable>

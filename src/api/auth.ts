...
const put = (url, data, options = {}) => {
    ...
    return Axios
        .put(url, data, {cancelToken: abort.token, ...options})
        .then(response => {
            clearTimeout(id);
            return response
        })
...
export const loginAdmin = () => login({username: 'lucas@marketsflow.com', password: 'UKengland10!'})
    .then(async p => {
        return p;
    });
export const logout = () => {
...
export const userUpload = async (data: UserUpload) => {
    ...
    return post(URLS.USER_UPLOAD.replace(':id', id), formData, {headers: {'Authorization': 'Bearer ' + token}})
        .then(async p => {
...
export const userUpdate = async (user: User,) => {
    const {id, token} = await getData();
    return put(URLS.USER_UPDATE.replace(':id', id), user, {headers: {'Authorization': 'Bearer ' + token}})
        .then(async p => {
